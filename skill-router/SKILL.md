...
| Address GitHub PR review comments | `gh-address-comments` |
| Diagnose + fix failing GitHub Actions CI | `gh-fix-ci` |

### Plugin / MCP development

| Intent | Skill |
|---|---|
| Build an MCP server | `build-mcp-server`, `anthropic-skills:mcp-builder`, `mcp-builder` |
...
| Write / debug Excel or Google Sheets formulas | `spreadsheet-formula-helper` |
...
| Analyze meeting transcripts for behavioral patterns + coaching | `meeting-insights-analyzer` |
...
| 60-90% token savings on Bash commands (installed: `rtk.exe` in `~/.local/bin`) | run `rtk <cmd>` directly — already injected into CLAUDE.md |
